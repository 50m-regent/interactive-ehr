...
def list_table_columns(
    ...
    *,
    ...
    with sqlite3.connect(uri, uri=True) as connection:
        rows = connection.execute(
            f'PRAGMA table_info("{_escape_identifier(table_name)}")'
        ).fetchall()
    ...
    sample_tables = {
        ...
        "慢性疾患外来_カルテ記載": pd.DataFrame(
            [
                {
                    "種別": "副作用",
                    "内容": "前回は立ちくらみなし。夜間頻尿が増え、SGLT2阻害薬との関連を確認予定。",
                },
                {
                    "種別": "服薬",
                    "内容": "朝薬は概ね内服。夕薬は週1回程度忘れるため一包化を相談。",
                },
                {
                    "種別": "処方調整",
                    "内容": "ARB増量はKとeGFRを再確認後。降圧不足ならCa拮抗薬増量も候補。",
                },
            ]
        ),
